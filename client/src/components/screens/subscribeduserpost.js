    ...
    return(
        <div className="home">
        {
            data.map(item=>{
                ...
                <h4 style={{padding:"5px"}}><Link to={item.postedby._id!=state._id?'/profile/'+item.postedby._id:'/profile'}>{item.postedby.name}</Link>{item.postedby._id==state._id && <i class="material-icons"
                style={{float:"right"}}
